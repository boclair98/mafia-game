...
낮이 되면 모든 생존자의 음성과 공개 채팅이 열립니다. 동시에 집중 발언자가 순서대로 바뀌므로 자유롭게 반박하면서도 각 참가자의 핵심 주장을 놓치지 않을 수 있습니다. 인원이 많으면 낮 시간이 자동으로 늘어납니다.

현재 발언자는 다음 권한을 가집니다.

- 화면 중앙 집중 조명과 남은 발언 시간 표시
- 해당 라운드에서 한 번만 가능한 `공식 진술 봉인`

나머지 생존자는 다음 행동을 할 수 있습니다.

- 자유 음성·채팅 토론에 참여하기
- 현재 집중 발언자에게 별도의 텍스트 질문 보내기
...
생존자는 자신을 제외한 한 명에게 투표할 수 있습니다. 봉인된 표는 공개 투표 호명 화면에 순서대로 쌓이며, 단독 최다 득표자가 없으면 그날은 피고 없이 종료됩니다.

### 7. 최후 변론과 판결

단독 최다 득표자는 즉시 처형되지 않습니다. 피고에게만 최후 변론권이 주어지고, 나머지는 변론이 끝난 뒤 `처형` 또는 `석방`을 선택합니다.

처형표가 석방표보다 많을 때만 처형됩니다. 동률은 석방입니다. 처형된 사람의 역할은 즉시 공개되지 않아 다음 날에도 추리가 이어집니다. 광대가 이 절차로 처형되면 다른 팀의 상태와 관계없이 광대가 단독 승리합니다.
...
| 낮 자유 토론 | 생존한 음성 참여자 전체 · 현재 집중 발언자 강조 |
| 시민 투표 | 생존한 음성 참여자 전체 |
| 최후 변론 | 피고 한 명 |
| 최종 판결 | 모두 자동 음소거 |
| 게임 종료 | 음성 참여자 전체 |
